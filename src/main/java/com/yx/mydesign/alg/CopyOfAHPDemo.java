...
public class CopyOfAHPDemo {
	public static double[] getWeightMatrix() {  
        /** a为N*N矩阵 */  
		//计算人、物、环境等不同因素对目标层即确定权重的影响
		double[][] a = new double[][]{{1,2,3},{0.5,1,1.5},{0.333,0.667,1}};
        //计算由人造成的空气中PM2.5，PM10，CO2的含量的影响
		double[][] a1 = new double[][]{{1,1/7,1/7,1/5},{7,1,1,3},{7,1,1,3},{5,1/3,1/3,1}};
		//计算由物造成的空气中PM2.5，PM10，CO2的含量的影响
		double[][] a2 = new double[][]{{1,3,3,5},{1/3,1,1,1/2},{1/3,1,1,1/2},{1/5,2,2,1}};
		//计算由环境污染造成的空气中PM2.5，PM10，CO2的含量的影响
		double[][] a3 = new double[][]{{1,1/5,1/5,1/3},{5,1,1,3},{5,1,1,3},{3,1/3,1/3,1}};
		int N = a[0].length;  
		int M = a1[0].length;
        ...
        CopyOfAHPDemo instance = CopyOfAHPDemo.getInstance();  
        ...
        weightsum[0] = round(weight[0]*weight1[0]+weight[1]*weight2[0]+weight[2]*weight3[0],4);
        weightsum[1] = round(weight[0]*weight1[1]+weight[1]*weight2[1]+weight[2]*weight3[1],4);
        weightsum[2] = round(weight[0]*weight1[2]+weight[1]*weight2[2]+weight[2]*weight3[2],4);
        weightsum[3] = round(weight[0]*weight1[3]+weight[1]*weight2[3]+weight[2]*weight3[3],4);
        System.out.println("方案层对目标层权重（甲醛、PM2.5,PM10;CO2）:"+Arrays.toString(weightsum));
        return weightsum;
    }
    // 单例  
    private static final CopyOfAHPDemo acw = new CopyOfAHPDemo();  
     ...
     * @return 
     */  
    public static CopyOfAHPDemo getInstance() {  
        return acw;  
    }  
